...
function ItemCard({ item }: ItemPreviewProps) {
	const { getItemName, getStatName, getSoleName, formatStatValue } =
		...
		<div className="flex flex-col w-full max-w-sm bg-[rgb(32,30,28)] border-beige border-1">
			...
			{(item.stats || item.soles) && (
				<div className="flex flex-col">
						...
						<div className="flex flex-col px-3 gap-3 mb-3">
							<div className="flex flex-col gap-1">
								{item.stats.map((stat, index) => (
									<div
										className={`flex flex-row text-justify text-sm text-gray-100`}
										key={index}
									>
					...
					{item.stats && item.soles && (
						<div className="h-px bg-gray-600 mx-3 mb-3"></div>
					)}
						...
						<div className="flex flex-col px-3 gap-3 mb-3">
							<div className="flex flex-col gap-1">
								{item.soles.map((sole, index) => (
									<div
										className={`flex flex-col text-yellow-600 text-sm`}
										key={index}
									>
									...
									>
										<div className="flex flex-col justify-center">
											<div
												...
												style={{
													border: `1px solid ${itemBgStyles[targetItem.color]}`,
												}}
											>
												<img
													src={targetItem.icon}
													alt={getItemName(targetItemId)}
													className="w-10 h-10 object-cover"
												/>
											</div>
											<div className="flex justify-center items-center gap-1">
												<img
													src={"/moba/items/gold-small.webp"}
													alt="gold"
						...
						<div className="flex flex-row gap-3">
							{item.recipe.map((recipeItemId) => {
								...
								return (
									<Link
										to={`/moba/items/$itemId`}
										params={{ itemId: recipeItemId }}
									>
										<div className="flex flex-col justify-center">
											<div
												className={`flex justify-center items-center w-12 h-12`}
												style={{
													border: `1px solid ${itemBgStyles[recipeItem.color]}`,
												}}
											>
												<img
													src={recipeItem.icon}
													alt={getItemName(recipeItemId)}
													className="w-10 h-10 object-cover"
												/>
											</div>
											<div className="flex justify-center items-center gap-1">
